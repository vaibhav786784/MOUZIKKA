...
class View:
    def __init__(self, top=None):
        ...
        self.song_scale.place(relx=0.200, rely=0.932, relwidth=0.146, relheight=0.0
                             , height=26, bordermode='ignore')
    ...
    def show_song_details(self):
        ...
        sng_name_str=self.song_name[0:ext_index]
        if(len(sng_name_str)>14):
            sng_name_str=sng_name_str[0:14]+". . ."
        self.songName.configure(text=sng_name_str)
    ...
    def load_previous_song(self):
        try:
            if hasattr(self,"sel_song_index_tuple")==False:
                raise NoSongSelectedError("Plaese Select A song")
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            mycolor = "#" + red + green + blue
            self.playList.config(fg=mycolor)
            messagebox.showinfo("FAVOURITES loaded!!!", "Songs loaded successfully from your favourites!!!")
    ...
    def remove_songs_from_favourites(self):
        remove_song_index_tuple = self.playList.curselection()
        try:
            if len(remove_song_index_tuple) == 0:
                raise NoSongSelectedError("Please select a song!!")
            song_name = self.playList.get(remove_song_index_tuple[0])
            result = self.my_player.remove_songs_from_favourites(song_name)
            if result.find("deleted")!=-1:
                self.playList.delete(remove_song_index_tuple[0])
            if self.isPlaying:
                if self.song_name == song_name:
                    self.stop_song()
            messagebox.showinfo("Remove song from favourites!!!", result)
        except(NoSongSelectedError)as ex1:
            messagebox.showerror("Error!", ex1)
        except(DatabaseError)as ex2:
